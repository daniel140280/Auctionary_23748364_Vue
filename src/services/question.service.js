...
const getQuestionsForItem = (item_id) => {
  return fetch(`http://localhost:3333/item/${item_id}/question`).then(
    (response) => {
      ...
      if (response.status === 404) {
        ...
        throw new Error(
          "Something went wrong. Unable to fetch questions for the item"
        );
      }
    }
  );
...
const answerQuestion = (question_id, answerText) => {
  ...
  return fetch(`http://localhost:3333/question/${question_id}`, {
    ...
    if (response.status === 403) {
      throw new Error(
        "Only the seller can answer questions for their own items."
      );
    }
  });
};
