    ...
    <div class="single-item-container">
      <div class="nav-back">
        <router-link to="/" class="btn-back"
          >← Back to All Listings</router-link
        >
      </div>
      ...
      <div v-else class="content-layout">
        ...
        <div class="main-content">
          ...
          <div class="item-header-card">
            <h1>{{ item.name }}</h1>
            <p class="seller-info">
              <span class="label">Seller:</span>{{ item.first_name }}
              {{ item.last_name }}
            </p>
          </div>
  ...
  methods: {
    fetchItemDetails() {
      ...
      CoreService.searchSingleItems(this.$route.params.item_id)
        ...
        .catch((err) => {
          this.error =
            err.message || "Item details failed to load. Please check the ID.";
        })
        .finally(() => {
          this.loading = false;
        });
    ...
    fetchQuestions() {
      QuestionService.getQuestionsForItem(this.$route.params.item_id)
        ...
        .catch((err) =>
          console.error("Encountered error fetching questions:", err)
        );
    },
    submitQuestion() {
